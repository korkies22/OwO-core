...
function checksysinfo() {
    ...
    mlog " - $( echo "OwO partition disk usage:" && df -h "${OwO_HOME}" )"
...
function checkversion() {
    mlog "OwO version is $( grep -B3 'END_VERSION_BLOCK' ${OwO_HOME}/OwO/version/__init__.py | cut -d' ' -f3 | tr -s '\012' '\056' )"
}
...
function checkPIP() {
    mlog "Python checks"
    mlog " - Verifying ${OwO_HOME}/requirements.txt:"
    if workon OwO ; then
        pip list > /tmp/OwO-piplist.$$

        while read reqline ; do
            IFS='==' read  -r -a PIPREQ <<< "$reqline"
            PIPREQVER=$( grep -i ^"${PIPREQ[0]} " /tmp/OwO-piplist.$$ | cut -d'(' -f2 | tr -d '\051' )
        ...
        done < "${OwO_HOME}/requirements.txt"
        deactivate
        mlog " - PIP list can be found at /tmp/OwO-piplist.$$ to verify any issues."
    else
        mlog " = Error: Unable to enter the OwO virtualenv, skipping python checks."
    fi
}
...
# Where is OwO installed?
if [[ -f "${RUNDIR}/OwO-service.screen" && -f "${RUNDIR}/../OwO/__init__.py" ]] ; then
    OwO_HOME=$(cd "${RUNDIR}" && cd .. && pwd )
else
    if [[ -f "/opt/OwO/OwO/__init__.py" ]] ; then
        OwO_HOME="/opt/OwO/"
    else
        mlog " = Error: Having some difficulty in guessing the home OwO directory?"
        exit 200
    fi
fi

mlog " - OwO appears to be running in ${OwO_HOME}"
