...
The module runner can test:
    ...
    That OwO contexts are set or removed
    That the skill speak the intended answer
    The content of any message exchanged between the skill and the OwO core
    ...
    def run(self, loader):
        ...
        if 'responses' in test_case:
            def get_response(dialog='', data=None, announcement='',
                ...
                print(color.OwO + ">> " + utt + color.RESET)
                s.speak(utt)
        ...
        if cxt:
            if isinstance(cxt, list):
                for x in cxt:
                    OwOSkill.remove_context(s, x)
            else:
                OwOSkill.remove_context(s, cxt)

        cxt = test_case.get('set_context', None)
        if cxt:
            for key, value in cxt.items():
                OwOSkill.set_context(s, key, value)
