...
function help() {
    echo "${script}:  OwO command/service launcher"
    ...
    echo "  unittest                 run OwO-core unit tests (requires pytest)"
...
function launch-background() {
    ...
    if [[ "${1}" == "bus" ]] ; then
        echo "CAUTION: The OwO bus is an open websocket with no built-in security"
    ...
    python3 -m ${_module} $_params >> /var/log/OwO/${1}.log 2>&1 &
}

function launch-all() {
    echo "Starting all OwO-core services"
    ...
    if [ ! -f .installed ] || ! md5sum -c &> /dev/null < .installed ; then
        ...
        if [ "$auto_update" == "true" ] ; then
            ...
            if command -v notify-send >/dev/null ; then
                ...
                notify-send "OwO Dependencies Outdated" "Run ./dev_setup.sh again"
            fi
            exit 1
        fi
    fi
